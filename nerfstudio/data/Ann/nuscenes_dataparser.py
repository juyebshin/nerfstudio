...
@dataclass
class NuscDataParserConfig(DataParserConfig):
    """Nerfstudio dataset config"""

    _target: Type = field(default_factory=lambda: Nusc)
...
@dataclass
class Nusc(DataParser):
    """Nusc DatasetParser"""

    config: NuscDataParserConfig
    downscale_factor: Optional[int] = None
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        if self.config.data.suffix == ".json":
            ...
            data_dir = self.config.data.parent.parent.parent # to check
        else:
            meta = load_from_json(self.config.data / f"hloc/{self.config.location}-c{self.config.centroid_name}" / "transforms.json")
        ...
        for frame in meta["frames"]:
            ...
            fname = self._get_fname(filepath.relative_to(data_dir), data_dir) # to change
            fnames.append(fname)
        ...
        for frame in frames:
            filepath = Path(frame["file_path"])
            fname = self._get_fname(filepath.relative_to(data_dir), data_dir)
        ...
        elif "camera_model" in meta["frames"][0]:
            camera_type = CAMERA_MODEL_TO_TYPE[meta["frames"][0]["camera_model"]]
        else:
        ...
        colmap_path = self.config.data / f"hloc/{self.config.location}-c{self.config.centroid_name}" / "sparse/0"
        ...
        if self.config.load_3D_points:
            if "ply_file_path" in meta:
                ply_file_path = data_dir / f"hloc/{self.config.location}-c{self.config.centroid_name}" / meta["ply_file_path"]
            ...
            if ply_file_path:
                sparse_points = self._load_3D_points(ply_file_path, transform_matrix, scale_factor) # keys: points3D_xyz, points3D_rgb
