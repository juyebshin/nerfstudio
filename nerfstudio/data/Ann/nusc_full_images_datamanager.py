...
@dataclass
class NuscFullImageDatamanagerConfig(DataManagerConfig):
    _target: Type = field(default_factory=lambda: NuscFullImageDatamanager)
    dataparser: AnnotatedDataParserUnion = field(default_factory=NuscDataParserConfig)
...
class NuscFullImageDatamanager(DataManager, Generic[TDataset]):
    """
    ...
    config: NuscFullImageDatamanagerConfig
    ...
    def dataset_type(self) -> Type[TDataset]:
        ...
        orig_class: Type[NuscFullImageDatamanager] = get_orig_class(self, default=None)  # type: ignore
        if type(self) is NuscFullImageDatamanager and orig_class is None:
            return default
        if orig_class is not None and get_origin(orig_class) is NuscFullImageDatamanager:
            return get_args(orig_class)[0]

        # For inherited classes, we need to find the correct type to instantiate
        for base in getattr(self, "__orig_bases__", []):
            if get_origin(base) is NuscFullImageDatamanager:
    ...
    def fixed_indices_eval_dataloader(self) -> List[Tuple[Cameras, Dict]]:
        ...
        data = deepcopy(self.eval_dataset) # self.cached_eval or eval_dataset
    ...
    def next_train(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        data = deepcopy(self.train_dataset[image_idx]) # self.cached_train or self.train_dataset
    ...
    def next_eval(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        if len(self.eval_unseen_cameras) == 0:
            self.eval_unseen_cameras = [i for i in range(len(self.eval_dataset))]
        data = deepcopy(self.eval_dataset[image_idx]) # self.cached_eval or self.eval_dataset
        data["image"] = data["image"].to(self.device)
        assert len(self.eval_dataset.cameras.shape) == 1, "Assumes single batch dimension"
