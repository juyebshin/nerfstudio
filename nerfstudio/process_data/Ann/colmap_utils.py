...
def run_colmap(
    ...
    # ]
    # feature_extractor_cmd.extend(["--image_list_path"] + [str(r) for r in references])
    ...
    with status(msg="[bold yellow]Running COLMAP feature extractor...", spinner="moon", verbose=verbose):
        # todo: check camera_mode=pycolmap.CameraMode.SINGLE
        pycolmap.extract_features(str(colmap_dir / 'database.db'), str(image_dir), [str(r) for r in references], camera_model=camera_model.value, device=pycolmap.Device.auto if gpu else pycolmap.Device.cpu)
    ...
    with status(msg="[bold yellow]Running COLMAP feature matcher...", spinner="runner", verbose=verbose):
        if matching_method == "vocab_tree":
            vocab_tree_filename = get_vocab_tree()
            pycolmap.match_vocabtree(str(colmap_dir / 'database.db'), matching_options=pycolmap.VocabTreeMatchingOptions(vocab_tree_path=vocab_tree_filename), device=pycolmap.Device.auto if gpu else pycolmap.Device.cpu)
        elif matching_method == "exhaustive":
            pycolmap.match_exhaustive(str(colmap_dir / 'database.db'), device=pycolmap.Device.auto if gpu else pycolmap.Device.cpu)
        elif matching_method == "sequential":
            pycolmap.match_sequential(str(colmap_dir / 'database.db'), device=pycolmap.Device.auto if gpu else pycolmap.Device.cpu)
    ...
    if reference_model_dir is not None:
        with status(
            ...
            reference_model = pycolmap.Reconstruction(reference_model_dir)
            pycolmap.triangulate_points(reference_model, str(colmap_dir / 'database.db'), str(image_dir), str(sparse_dir), refine_intrinsics=refine_intrinsics)
    else:
        with status(
            msg="[bold yellow]Running COLMAP bundle adjustment... (This may take a while)",
            spinner="circle",
            ...
            if colmap_version >= 3.7:
                mapper = pycolmap.incremental_mapping(str(colmap_dir / 'database.db'), str(image_dir), str(sparse_dir), pycolmap.IncrementalPipelineOptions(ba_global_function_tolerance=1e-6))
            else:
                mapper = pycolmap.incremental_mapping(str(colmap_dir / 'database.db'), str(image_dir), str(sparse_dir))
        ...
        if refine_intrinsics:
            with status(msg="[bold yellow]Refine intrinsics...", spinner="dqpb", verbose=verbose):
                ...
                pycolmap.bundle_adjustment(mapper[0], pycolmap.BundleAdjustmentOptions(refine_principal_point=1))
...
def colmap_to_json(
    ...
    for im_id, im_data in im_id_to_image.items():
        ...
        w2c = np.concatenate([rotation, translation], 1) # shape [3, 4]
        w2c = np.concatenate([w2c, np.array([[0, 0, 0, 1]])], 0) # shape [4, 4]
        ...
        frame.update(parse_colmap_camera_params(cam))
        if camera_mask_path is not None:
